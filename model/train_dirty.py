...
def train(model, optimizer, train_loader, val_loader, epochs=10, device='cpu'):
    """
    ...
    torch.save(model.state_dict(), '../data/mttx_cnn4_final.pth')
    print('Model saved!')
...
if __name__ == '__main__':
    ...
    epochs = 10
    lr = 1e-3
    modelCNN2 = nn.Sequential(  # We can construct the new model by adding our modules here
        CNN_custom(3, channel1, channel1),
        CNN_custom(channel1, channel2, channel2),
        CNN_custom(channel2, channel2, channel3),
        # CNN_custom(channel3, channel3, channel4),
        nn.Flatten(),
        nn.Linear(in_features=6 * 6 * channel3, out_features=out_features))  # 48/2/2/2/2 = 3

    # Train
    # train(modelCNN2, optimiser, train_loader, test_loader, epochs=10, device='cpu')

    # final_accuracy = accuracy(modelCNN2, test_loader, device='cpu')
    ...
    model_cnn4 = nn.Sequential(
            ...
            nn.Linear(channels[0], out_features),
            nn.Softmax(dim=1)
    )
    optimiser = torch.optim.Adam(model_cnn4.parameters(), lr=lr)
    #optimiser_cnn3 = torch.optim.Adam(model_cnn4.parameters(), lr=lr)
    train(model_cnn4, optimiser, train_loader, test_loader, epochs=10, device='cpu')
    final_accuracy = accuracy(model_cnn4, test_loader, device='cpu')
    print(f'Final Accuracy on Test Set: {final_accuracy:.2f}%')

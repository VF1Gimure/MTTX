...
class DatasetSaver_frames:
    def __init__(self, dataset_name="KnightX_exp_dataset", category="neutral", max_frames=100):
        self.category = category
    ...
    def save_frame(self, frame, detections):
        ...
        # Save full frame
        frame_filename = os.path.join(self.full_frame_path, f"frame_{self.frame_count}.jpg")
        ...
        if len(boxes) > 0:
            ...
            # Ensure the face is valid before saving
            if cropped_face.size > 0:
                face_filename = os.path.join(self.cropped_face_path, f"face_{self.frame_count}.jpg")
                cv2.imwrite(face_filename, cropped_face)
